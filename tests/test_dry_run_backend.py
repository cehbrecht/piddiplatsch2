...
def test_plugin_uses_jsonl_backend_in_dry_run():
    ...
    assert isinstance(getattr(plugin.handle_backend, "backend", None), JsonlHandleBackend)
...
def test_pipeline_initializes_jsonl_backend_in_dry_run():
    ...
    assert isinstance(getattr(pipeline.processor.handle_backend, "backend", None), JsonlHandleBackend)
